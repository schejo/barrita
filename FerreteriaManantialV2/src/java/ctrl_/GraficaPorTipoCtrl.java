...
import org.ngi.zhighcharts.ZHighCharts;
  ...
  public class GraficaPorTipoCtrl extends GenericForwardComposer {
  ...
  //  private Datebox Fecha_Final;
    ...
    public void Grafica1(String Fecha_inicial) throws SQLException{
        GraficaPorTipoDal dataBase = new GraficaPorTipoDal();

        grafica1.setType("column");
        grafica1.setTitle("Productos por Tipo");
        grafica1.setSubTitle("Fechas "+Fecha_inicial +"");

        grafica1.setyAxisOptions("{ "
                + "min:0"
                + "}");
        grafica1.setYAxisTitle("Total por Tipo");
        ...
        List<GraficaPorTipoMd> data = dataBase.Grafica1(Fecha_inicial);
        String categoria = "";

        int i = 0;
        
        for (GraficaPorTipoMd valor : data) {
            if (categoria.equals("")) {
                categoria = "'" + valor.getTipo()+ "'";
            } else {
                categoria = categoria + "," + "'" + valor.getTipo()+ "'";
            }
            model.addValue("Total de Productos", i, Integer.parseInt(valor.getSumatipo()));
              i++;
        }
    ...
    public void Grafica2(String fecha_inicial) throws SQLException {
        ...
        grafica2.setTitle("PORCENTAJE DE PRODUCTOS POR TIPO");
